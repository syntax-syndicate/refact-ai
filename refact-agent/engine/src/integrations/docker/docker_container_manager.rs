...
use crate::integrations::docker::docker_ssh_tunnel_utils::{
    ssh_tunnel_open, SshTunnel, ssh_tunnel_check_status,
};
...
impl IntegrationSession for DockerContainerSession {
    ...
    fn try_stop(
        ...
        Box::new(async move {
            ...
            let docker_session = container_session
                .as_any_mut()
                .downcast_ref::<DockerContainerSession>()
...
pub async fn docker_container_check_status_or_start(
    ...
    let docker_container_session_maybe = {
        ...
        gcx_locked
            ...
            .get(&get_session_hashmap_key("docker", chat_id))
            .cloned()
    };
    ...
    match docker_container_session_maybe {
        Some(docker_container_session) => {
            ...
            let docker_container_session = docker_container_session_locked
                .as_any_mut()
                .downcast_mut::<DockerContainerSession>()
                ...
                DockerContainerConnectionEnum::SshTunnel(ssh_tunnel) => {
                    match ssh_tunnel_check_status(ssh_tunnel).await {
                        ...
                        Err(e) => {
                            ...
                            let ssh_config = docker
                                .settings_docker
                                .get_ssh_config()
                                .ok_or_else(|| "No ssh config for docker container".to_string())?;
                            docker_container_session.connection =
                                DockerContainerConnectionEnum::SshTunnel(
                                    ssh_tunnel_open(&mut ssh_tunnel.forwarded_ports, &ssh_config)
                                        .await?,
                                );
                        }
                    }
            ...
            docker_container_session.last_usage_ts = SystemTime::now()
                .duration_since(SystemTime::UNIX_EPOCH)
                .unwrap()
                .as_secs();
            Ok(())
        ...
        None => {
            ...
            ports_to_forward.insert(
                0,
                Port {
                    published: "0".to_string(),
                    target: LSP_PORT.to_string(),
                },
            );

            let container_id = docker_container_create(
                &docker,
                &isolation,
                chat_id,
                &ports_to_forward,
            ...
            let exposed_ports = docker_container_get_exposed_ports(
                &docker,
                &container_id,
                &ports_to_forward,
                gcx.clone(),
            ...
            .await?;
            let host_lsp_port = exposed_ports
                .iter()
                .find(|p| p.target == LSP_PORT)
                .ok_or_else(|| "No LSP port exposed".to_string())?
                .published
                .clone();

            let connection = match ssh_config_maybe {
                Some(ssh_config) => {
                    let mut ports_to_forward_through_ssh = exposed_ports
                        .into_iter()
                        .map(|exposed_port| {
                            let matched_external_port = isolation
                                .ports
                                .iter()
                                .find(|configured_port| {
                                    configured_port.target == exposed_port.target
                                })
                                .map_or_else(
                                    || "0".to_string(),
                                    |forwarded_port| forwarded_port.published.clone(),
                                );
                            Port {
                        ...
                        .collect::<Vec<_>>();
                    let ssh_tunnel =
                        ssh_tunnel_open(&mut ports_to_forward_through_ssh, &ssh_config).await?;
                    DockerContainerConnectionEnum::SshTunnel(ssh_tunnel)
                }
            ...
            docker_container_sync_workspace(
                gcx.clone(),
                &docker,
                &isolation,
                &container_id,
            ...
            let session: Arc<AMutex<Box<dyn IntegrationSession>>> =
                Arc::new(AMutex::new(Box::new(DockerContainerSession {
                    container_id,
                    connection,
                    last_usage_ts: SystemTime::now()
                        .duration_since(SystemTime::UNIX_EPOCH)
                        .unwrap()
                        .as_secs(),
                    session_timeout_after_inactivity: Duration::from_secs(
                        60 * isolation.keep_containers_alive_for_x_minutes,
                    ),
            ...
            gcx_locked
                .integration_sessions
                .insert(get_session_hashmap_key("docker", chat_id), session);
            Ok(())
        }
    }
}
...
pub async fn docker_container_get_host_lsp_port_to_connect(
    ...
    match docker_container_session_maybe {
        ...
        None => Err("Docker container session not found, cannot get host port".to_string()),
    }
}
...
async fn docker_container_create(
    ...
    let host_lsp_path = format!(
        "{}/refact-lsp",
        get_host_cache_dir(gcx.clone(), &docker.settings_docker).await
    );
    ...
    let ports_to_forward_as_arg_list = ports_to_forward
        .iter()
        .map(|p| format!("--publish={}:{}", p.published, p.target))
        .collect::<Vec<_>>()
        .join(" ");
    ...
    let (run_output, _) = docker
        .command_execute(&run_command, gcx.clone(), true, true)
        .await?;
...
async fn get_host_cache_dir(
    gcx: Arc<ARwLock<GlobalContext>>,
    settings_docker: &SettingsDocker,
) -> String {
...
async fn docker_create_network_if_not_exists(
    gcx: Arc<ARwLock<GlobalContext>>,
    docker: &ToolDocker,
    network_name: &str,
) -> Result<(), String> {
    ...
    let (network_ls_output, _) = docker
        .command_execute(&network_ls_command, gcx.clone(), true, true)
        .await?;
    if !network_ls_output.contains(network_name) {
        let network_create_command = format!("network create {quoted_network_name}");
        let (_network_create_output, _) = docker
            .command_execute(&network_create_command, gcx.clone(), true, true)
            .await?;
    }
    Ok(())
}
...
async fn docker_container_sync_config_folder(
    ...
    let container_home_dir =
        docker_container_get_home_dir(docker, container_id, gcx.clone()).await?;
    ...
    docker_container_copy(
        ...
        container_id,
        &temp_dir_path,
        &format!("{container_home_dir}/.config/"),
    )
    .await?;
    docker_container_copy(
        docker,
        ...
        container_id,
        &config_dir_string,
        &format!("{container_home_dir}/.config/refact/"),
    )
    .await?;
    ...
    if !integrations_yaml.is_empty() {
        docker_container_copy(
            ...
            container_id,
            &integrations_yaml,
            &format!("{container_home_dir}/.config/refact/integrations.yaml"),
        )
        .await?;
    }
    if !variables_yaml.is_empty() {
        docker_container_copy(
            ...
            container_id,
            &variables_yaml,
            &format!("{container_home_dir}/.config/refact/variables.yaml"),
        )
        .await?;
    }
    if !secrets_yaml.is_empty() {
        docker_container_copy(
            ...
            container_id,
            &secrets_yaml,
            &format!("{container_home_dir}/.config/refact/secrets.yaml"),
        )
        .await?;
    }
    if !indexing_yaml.is_empty() {
        docker_container_copy(
            ...
            container_id,
            &indexing_yaml,
            &format!("{container_home_dir}/.config/refact/indexing.yaml"),
        )
        .await?;
    }
    if !privacy_yaml.is_empty() {
        docker_container_copy(
            ...
            container_id,
            &privacy_yaml,
            &format!("{container_home_dir}/.config/refact/privacy.yaml"),
        )
        .await?;
    }
...
async fn docker_container_copy(
    ...
    let cp_command = format!(
        "container cp {} {}:{}",
        shell_words::quote(local_path),
        container_id_or_name,
        shell_words::quote(remote_path)
    );
    docker
        .command_execute(&cp_command, gcx.clone(), true, true)
        .await?;
    Ok(())
}
...
async fn docker_container_get_home_dir(
    ...
    let inspect_config_command =
        "container inspect --format '{{json .Config}}' ".to_string() + container_id;
    let (inspect_config_output, _) = docker
        .command_execute(&inspect_config_command, gcx.clone(), true, true)
        .await?;

    let config_json: serde_json::Value = serde_json::from_str(&inspect_config_output)
        .map_err(|e| format!("Error parsing docker config: {}", e))?;

    if let Some(home_env) = config_json
        .get("Env")
...
async fn docker_container_start(
    ...
    docker
        .command_execute(&start_command, gcx.clone(), true, true)
        .await?;

    // If docker container is not running, print last lines of logs.
    let inspect_command =
        "container inspect --format '{{json .State.Running}}' ".to_string() + container_id;
    let (inspect_output, _) = docker
        .command_execute(&inspect_command, gcx.clone(), true, true)
        .await?;
    if inspect_output.trim() != "true" {
        let (logs_output, _) = docker
            .command_execute(
                &format!("container logs --tail 10 {container_id}"),
                gcx.clone(),
                true,
                true,
            )
...
async fn docker_container_sync_workspace(
    ...
    if !container_workspace_folder.ends_with("/") {
        container_workspace_folder.push('/');
    }

    let temp_tar_file = tempfile::Builder::new()
        .suffix(".tar")
        .tempfile()
        .map_err(|e| format!("Error creating temporary tar file: {}", e))?
        .into_temp_path();
    let tar_file_name = temp_tar_file
        .file_name()
        .unwrap_or_default()
        .to_string_lossy()
        .to_string();
    let tar_async_file = tokio::fs::File::create(&temp_tar_file)
        .await
    ...
    tar_builder
        .finish()
        .await
        .map_err(|e| format!("Error finishing tar archive: {}", e))?;

    docker_container_copy(
        docker,
        gcx.clone(),
        container_id,
        &temp_tar_file.to_string_lossy(),
    ...
    let sync_files_post = SyncFilesExtractTarPost {
        tar_path: format!(
            "{}/{}",
            container_workspace_folder.trim_end_matches('/'),
            tar_file_name
        ),
    ...
    tokio::fs::remove_file(&temp_tar_file)
        .await
        .map_err(|e| format!("Error removing temporary archive: {}", e))?;

    info!("Workspace synced successfully.");

    const ENCODE_SET: &percent_encoding::AsciiSet =
        &percent_encoding::NON_ALPHANUMERIC.remove(b'/');

    let container_workspace_folder_url = Url::parse(&format!(
        "file://{}",
    ...
    http_post(
        &format!("http://localhost:{lsp_port_to_connect}/v1/lsp-initialize"),
        &initialize_post,
    )
    .await?;
...
async fn docker_container_get_exposed_ports(
    ...
    let inspect_command =
        "inspect --format '{{json .NetworkSettings.Ports}}' ".to_string() + container_id;
    let (inspect_output, _) = docker
        .command_execute(&inspect_command, gcx.clone(), true, true)
        .await?;
...
async fn docker_container_kill(
    ...
    info!("Stopping and removing docker container {container_id}.");
    docker
        .command_execute(
